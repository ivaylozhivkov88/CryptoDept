...
class AggregateWhaleActivityUseCase @Inject constructor(
    ...
    private suspend fun fetchEthWhales(ethPrice: Double, limit: Int): List<WhaleTransactionV2> {
        val apiKey = BuildConfig.ETHERSCAN_API_KEY
        if (apiKey.isBlank()) return emptyList()
        
        val results = mutableListOf<WhaleTransactionV2>()
        KnownWallets.ETH_WALLETS.take(3).forEach { wallet ->
            try {
                ...
                if (response.status == "1") {
                    response.result.forEach { tx ->
                        ...
                        results.add(WhaleTransactionV2(
                            hash = tx.hash,
                            blockchain = "ethereum",
                            symbol = "ETH",
                            amount = amount,
    ...
    private suspend fun fetchBtcWhales(btcPrice: Double, limit: Int): List<WhaleTransactionV2> {
        val results = mutableListOf<WhaleTransactionV2>()
        KnownWallets.BTC_ADDRESSES.take(2).forEach { wallet ->
            try {
                val txs = mempool.getAddressTransactions(wallet.address)
                txs.take(limit).forEach { tx ->
    ...
    private suspend fun fetchSolWhales(solPrice: Double, limit: Int): List<WhaleTransactionV2> {
        val apiKey = BuildConfig.HELIUS_API_KEY
        if (apiKey.isBlank()) return emptyList()
        val results = mutableListOf<WhaleTransactionV2>()
        KnownWallets.SOL_WALLETS.take(2).forEach { wallet ->
            try {
                val txs = helius.getAddressTransactions(address = wallet.address, apiKey = apiKey, limit = limit)
                txs.forEach { tx ->
                    val transfer = tx.nativeTransfers?.firstOrNull() ?: return@forEach
