    ...
    constructor(
        ...
        suspend fun evaluate(
            ...
            withContext(Dispatchers.Default) {
                ...
                val rrRatio =
                    if (kotlin.math.abs(entryPrice - stopLoss) > 0) {
                ...
                val mtf =
                    try {
                        mtfAnalyzer.analyze(coin)
                    } catch (e: Exception) {
                        null
                    }
                val higherTFsBullish =
                    mtf
                        ?.timeframes
                ...
                checklist.add(
                    ChecklistItem(
                        "TREND",
                        "Higher timeframes (4H+) aligned with trade",
                        if (isLong) higherTFsBullish >= 2 else higherTFsBullish <= 1,
                        true,
                    ),
                )
                checklist.add(
                    ChecklistItem(
                        "TREND",
                        "Daily trend supports direction",
                ...
                checklist.add(
                    ChecklistItem(
                        "CONFLUENCE",
                        "RSI supports entry",
                        if (isLong) currentRsi < 50 else currentRsi > 50,
                        false,
                    ),
                )
                checklist.add(
                    ChecklistItem(
                        ...
                        if (isLong) fundingRate < 0.05 else fundingRate > 0.03,
                        false,
                    ),
                )
                checklist.add(
                    ChecklistItem(
                        ...
                        if (isLong) fearGreedIndex < 40 else fearGreedIndex > 65,
                        false,
                    ),
                )
                ...
                checklist.add(
                    ChecklistItem(
                        "MARKET",
                        "Overall Risk Score acceptable (<70)",
                        riskScore < 70,
                        false,
                    ),
                )
                checklist.add(
                    ChecklistItem(
                        "MARKET",
                        "Not trading against extreme sentiment",
                ...
                // Verdict mapping (10-point scale):
                // 8-10/10 → STRONG_SETUP (excellent conditions)
                // 6-7/10  → GOOD_SETUP (good conditions, proceed normally)
                // 4-5/10  → PROCEED_CAUTION (marginal, reduce position size)
                // <4/10   → AVOID (poor conditions or critical fails)
                val verdict =
                    when {
