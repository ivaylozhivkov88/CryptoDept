...
class PredictionAccuracyTracker @Inject constructor(
    ...
    suspend fun recordPrediction(
        ...
        val entity = PredictionAccuracyEntity(
            coinId = coinId,
            model = model,
    ...
    suspend fun verifyPredictions() = withContext(Dispatchers.IO) {
        try {
            ...
            unverified.forEach { prediction ->
                try {
                    val priceAtPrediction = repository.getPriceAtTimestamp(
                        prediction.coinId,
                    ...
                    Log.d("CryptoDept_AccuracyTracker",
    ...
    fun getModelAccuracy(model: String): Flow<Float> {
        return dao.getModelAccuracy(model).map { accuracy ->
            (accuracy ?: 0.0).coerceIn(0.0, 1.0).toFloat()
        }
    }

    fun getOverallAccuracy(): Flow<Float> {
        return dao.getOverallAccuracy().map { accuracy ->
            (accuracy ?: 0.0).coerceIn(0.0, 1.0).toFloat()
        }
    }
}
