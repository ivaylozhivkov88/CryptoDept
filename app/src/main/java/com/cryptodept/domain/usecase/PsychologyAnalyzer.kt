...
@Singleton
class PsychologyAnalyzer @Inject constructor(
    private val journalDao: TradeJournalDao
) {
    suspend fun analyzeSession(): SessionStats = withContext(Dispatchers.Default) {
        ...
        for (trade in sortedByTime) {
            if (trade.status == "CLOSED_LOSS") consecutiveLosses++
            else break
        }
        for (trade in sortedByTime) {
            if (trade.status == "CLOSED_WIN") consecutiveWins++
            else break
        }

        // Avg time between trades (minutes)
        val avgTime = if (allTrades.size >= 2) {
            val times = allTrades.mapNotNull { it.entryTime }
                .sortedDescending().zipWithNext { a, b -> (a - b) / 60000 }
        ...
        val tiltScore = minOf(100, (consecutiveLosses * 20) +
