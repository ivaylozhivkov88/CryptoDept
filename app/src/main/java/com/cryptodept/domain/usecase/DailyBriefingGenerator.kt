...
class DailyBriefingGenerator
    ...
    constructor() {
        /**
         * Formats number as millions with suffix "M" (e.g., 15.2M)
         */
        private fun formatMillions(value: Double): String = String.format(Locale.ENGLISH, "%.1f", value) + "M"

        /**
         * Formats price with thousands separator (e.g., 43,250.50)
         */
        private fun formatPrice(value: Double): String = String.format(Locale.ENGLISH, "%,.0f", value)

        data class DailyBriefing(
            val date: String,
            val generatedAt: Long,
            val marketSentence: String, // 1-sentence summary
        ...
        fun generate(
            ...
            val keyMetrics =
                listOf(
                    ...
                    BriefingMetric(
                        ...
                        null,
                        when {
                            fundingRate > 0.05 -> "BEARISH"
                            fundingRate < -0.02 -> "BULLISH"
                            else -> "NEUTRAL"
                        },
                    ),
                    BriefingMetric(
                        ...
                        null,
                        when {
                            fearGreedIndex > 70 -> "BEARISH"
                            fearGreedIndex < 30 -> "BULLISH"
                            else -> "NEUTRAL"
                        },
                    ),
                    BriefingMetric(
                        "EXCHANGE INFLOWS",
                        "${if (exchangeInflowChange > 0) "↑" else "↓"} ${String.format(
                            Locale.ENGLISH,
                            "%.1f",
                            Math.abs(exchangeInflowChange),
                        )}%",
                        null,
                        if (exchangeInflowChange > 15) {
                            "BEARISH"
                        } else if (exchangeInflowChange < -15) {
                            "BULLISH"
                        } else {
                            "NEUTRAL"
                        },
            ...
            if (exchangeInflowChange > 30) {
                alerts.add(
                    BriefingAlert(
                        AlertSeverity.WARNING,
                        "EXCHANGE INFLOW SPIKE",
                        "Exchange inflows up ${String.format(
                            Locale.ENGLISH,
                            "%.0f",
                            exchangeInflowChange,
        ...
        private fun buildMarketSentence(
            price: Double,
            change: Double,
            risk: RiskScoreEngine.RiskScore,
            fg: Int,
        ): String {
