 ...
 */
class HurstExponentCalculator @Inject constructor() {
     ...
     */
    fun interpret(hurst: Float, currentTrend: Direction): ModelVote {
        return when {
            // H > 0.55: Трендът е стабилен (Positive Autocorrelation)
            hurst > 0.55f -> ModelVote(
            ...
            hurst < 0.45f -> {
                val reversalDir = when (currentTrend) {
