...
@HiltViewModel
class MTFViewModel @Inject constructor(
    private val mtfAnalyzer: MultiTimeframeAnalyzer,
    private val repository: CryptoRepository,
    private val chartRepository: ChartRepository
) : ViewModel() {

    val trackedCoins: StateFlow<List<String>> = repository.getTrackedCoinPrices()
            ...
            try {
                ...
                val result = withContext(Dispatchers.Default) {
                    if (coinId == "ALL") {
                        ...
                        MTFConsensus(
                            timeframes = allResults.first().timeframes.map { tf ->
                                val tfSignals = allResults.map { it.timeframes.find { it2 -> it2.timeframe == tf.timeframe } }
                                val tfBullish = tfSignals.count { it?.overallSignal == OverallSignal.STRONG_BUY || it?.overallSignal == OverallSignal.BUY }
                                val tfBearish = tfSignals.count { it?.overallSignal == OverallSignal.STRONG_SELL || it?.overallSignal == OverallSignal.SELL }
                            ...
                            tradingBias = if (avgBullish > avgBearish) "BULLISH" else if (avgBearish > avgBullish) "BEARISH" else "NEUTRAL"
                        )
                    } else {
...
sealed class MTFUiState {
    ...
    data class Success(val consensus: MTFConsensus) : MTFUiState()
    data class Error(val message: String) : MTFUiState()
}
