...
@HiltViewModel
class EntryAnalyzerViewModel @Inject constructor(
    ...
    val entryData: StateFlow<EntryAnalysisUiState> = combine(
        _selectedCoin,
        refreshTrigger
    ) { coin, _ -> coin }
        .flatMapLatest { coin ->
            flow {
                ...
                try {
                    val result = withContext(Dispatchers.Default) {
                        ...
                        val analysis = calculator.analyze(
                            coin = coin,
...
sealed class EntryAnalysisUiState {
    ...
    data class Error(val message: String) : EntryAnalysisUiState()
}
