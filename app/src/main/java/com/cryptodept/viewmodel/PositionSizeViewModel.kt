...
class PositionSizeViewModel @Inject constructor(
    ...
    fun setPortfolioSize(value: String) { _portfolioSize.value = value; calculate() }
    fun setRiskPercent(value: String) { _riskPercent.value = value; calculate() }
    fun setEntryPrice(value: String) { _entryPrice.value = value; calculate() }
    fun setStopLoss(value: String) { _stopLoss.value = value; calculate() }
    fun setTakeProfit(value: String) { _takeProfit.value = value; calculate() }
    ...
    private fun calculate() {
        val portfolio = _portfolioSize.value.toDoubleOrNull() ?: run {
            _uiState.value = PositionSizeUiState.Error("INVALID PORTFOLIO SIZE")
            return
        }
        val entry = _entryPrice.value.toDoubleOrNull() ?: run {
            _uiState.value = PositionSizeUiState.Error("INVALID ENTRY PRICE")
            return
        }
        val sl = _stopLoss.value.toDoubleOrNull() ?: run {
...
sealed class PositionSizeUiState {
    ...
    data class Error(val message: String) : PositionSizeUiState()
}
