...
sealed class IndicatorsUiState {
    ...
    data class Success(val results: List<IndicatorScanResult>) : IndicatorsUiState()
    data class Error(val message: String) : IndicatorsUiState()
}
...
class IndicatorsViewModel @Inject constructor(
    ...
    fun scanIndicators() {
        viewModelScope.launch {
            ...
            try {
                val scanJobs = TRACKED_COINS.map { coinId ->
                    async {
