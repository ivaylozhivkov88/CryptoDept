...
class JournalViewModel
    ...
    constructor(
        ...
        val allTrades: StateFlow<List<TradeJournal>> =
            uiState
                .map {
                    if (it is JournalUiState.Success) it.trades else emptyList()
                }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

        val journalStats: StateFlow<JournalStats> =
            allTrades
                .map { trades ->
                    ...
                    val avgRR =
                        if (closed.isEmpty()) {
                            ...
                            closed.mapNotNull { it.riskRewardActual }.average().let {
